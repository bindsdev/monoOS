//! Physical memory manager implemented using a bitmap.
...
impl PMAlloc {
    ...
    pub(super) fn new(memmap: &'static mut [NonNullPtr<MemmapEntry>]) -> Self {
            ...
            .filter(|e| e.typ != MemoryMapEntryType::Usable)
        {
            ...
            pmm.mark_contiguous_frames(base, end, true);
        }
    ...
    fn mark_contiguous_frames(&mut self, base: u64, end: u64, allocated: bool) {
        let base = base / PMAlloc::FRAME_SIZE;
        let end = end.div_ceil(PMAlloc::FRAME_SIZE);
    ...
    fn allocate(&mut self) -> Option<PhysAddr> {
        ...
        Some(PhysAddr::new(first_free_bit as u64 * PMAlloc::FRAME_SIZE))
    }

    /// Deallocate the frame starting at `frame_addr`.
    fn deallocate(&mut self, frame_addr: PhysAddr) {
        let frame = frame_addr.as_u64() / PMAlloc::FRAME_SIZE;
        self.mark_frame(frame as usize, false);
    }
}

// SAFETY: the physical memory manager returns unique, usable frames.
unsafe impl FrameAllocator<Size4KiB> for PMAlloc {
...
impl FrameDeallocator<Size4KiB> for PMAlloc {
...
static PMALLOC: Once<Mutex<PMAlloc>> = Once::new();

/// Get a handle to the physical memory manager.
///
/// # Panics
///
/// This function will panic if the physical memory manager has not been initialized.
pub(super) fn get_pmm() -> MutexGuard<'static, PMAlloc> {
    PMALLOC
        .get()
...
pub(super) fn init(memmap: &'static mut [NonNullPtr<MemmapEntry>]) {
    PMALLOC.call_once(|| Mutex::new(PMAlloc::new(memmap)));
}
...
mod tests {
    ...
    fn allocation() {
        ...
        let frame_idx = (frame.start_address().as_u64() / PMAlloc::FRAME_SIZE) as usize;
        assert!(pmm.is_frame_used(frame_idx));
    }
}
